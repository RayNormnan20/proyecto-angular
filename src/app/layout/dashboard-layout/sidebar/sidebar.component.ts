    ...
    >
      ...
      <div class="flex items-center justify-between h-16 px-6 bg-[#1a1c23] border-b border-gray-800 shrink-0">
        <div class="flex items-center space-x-2">
            ...
            <h1 class="text-xl font-bold tracking-wide text-white">{{ brandingService.appName() }}</h1>
        </div>
      ...
      <nav class="flex-1 overflow-y-auto px-4 pb-4 space-y-1 mt-6">
        ...
        <ng-container *ngIf="authService.hasRole('admin') || authService.hasPermission('VER_CONFIGURACION') || authService.hasPermission('VER_ENVIOS') || authService.hasPermission('VER_LOGS_EMAIL') || authService.hasPermission('VER_TESTIMONIOS') || authService.hasPermission('VER_METODOS_PAGO') || authService.hasPermission('GESTIONAR_BANNERS_HOME') || authService.hasPermission('VER_CUPONES')">
            <!-- Botón Padre -->
            <button 
            ...
            >
                ...
                <a 
                    *ngIf="authService.hasRole('admin') || authService.hasPermission('VER_CUPONES')"
                    routerLink="/dashboard/coupon-settings" 
                    routerLinkActive="text-white border-l-4 border-indigo-500 bg-[#252830]"
                    (click)="close.emit()"
                    class="flex items-center pl-12 pr-4 py-2 text-sm text-gray-400 hover:text-white hover:bg-[#252830] transition-colors border-l-4 border-transparent"
                >
